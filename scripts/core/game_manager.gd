...
func _handle_special_command(cmd: String, args: Array) -> bool:
	match cmd:
		...
		"status":
			...
			s += "Credits:  $%d\n" % p.credits
			s += "Rep:      %d\n" % p.reputation
			s += "Tasks:    %d active | %d completed\n" % [p.active_tasks.size(), p.completed_tasks_count]
		
			var bonuses = NodeController.get_node_bonuses(p.current_ip)
			if not bonuses.is_empty():
				s += "\n[color=#FFFF00]Node Bonuses:[/color]\n"
				for type in bonuses:
					s += "  • %s: +%d\n" % [type.capitalize(), bonuses[type]]
		...
		"map":
			_handle_map(args)
		"upgrade_base":
			_handle_upgrade_base(args)
		"join_event":
			_handle_join_event(args)
		"skip_tutorial":
...
func _handle_build_base(args: Array):
	# delegate to the C# server logic
	var result = await ServerInterface.ExecuteCommand("build_base " + (args[0] if args.size() > 0 else ""))
	if result.has("error"):
		ui.append_terminal_output("[ERROR] " + result["error"], "#FF4444")
		return
	ui.append_terminal_output(result["output"], "#00FF00")
	# Update local pref for immediate feedback, though server is the source of truth
	ZeroDayCore.save_pref("base_location", ZeroDayCore.current_player.current_ip)
	ZeroDayCore.notification_received.emit("BASE", "Base Established", "Your base is at " + ZeroDayCore.current_player.current_ip)
...
func _handle_claim(args: Array):
	if args.is_empty():
		...
		return
	var target = args[0]
	var result = await ServerInterface.ExecuteCommand("claim " + target)
	if result.has("error"):
		ui.append_terminal_output("[ERROR] " + result["error"], "#FF4444")
		return
	ui.append_terminal_output(result["output"], "#00FF00")
	ZeroDayCore.notification_received.emit("CLAIM", "Node Claimed", target + " added to your subnet")
	ZeroDayCore.node_claimed.emit(target, ZeroDayCore.current_player.username)

func _handle_base_status():
	# Use ServerInterface to get the most accurate base data
	var result = await ServerInterface.ExecuteCommand("base_status")
	if result.has("error"):
		ui.append_terminal_output("[ERROR] " + result["error"], "#FF4444")
		return
	ui.append_terminal_output(result["output"], "#cccccc")
...
func _handle_map(args: Array):
	var subcmd = args[0] if args.size() > 0 else "show"
	match subcmd:
		"show":
			ui.show_world_map()
			ui.append_terminal_output("[color=#00FFFF]╔══════════════════════════════════════╗\n║  WORLD MAP: %s                    ║\n╚══════════════════════════════════════╝[/color]" % ZeroDayCore.current_player.current_ip, "#00FFFF")
			var p = ZeroDayCore.current_player
			var info = "[color=#00FF00]📍 %s | Level %d | HP: %d/%d[/color]" % [p.username, p.level, 100 + p.level * 10, 100 + p.level * 10]
			ui.append_terminal_output(info, "#cccccc")
			ui.append_terminal_output("Legend: [color=#00FFFF]⬡[/color] Router  [color=#FFD700]₿[/color] Crypto Mine  [color=#4488FF]◈[/color] Data Center", "#808080")
			ui.append_terminal_output("[color=#FF4444]☠[/color] Virus  [color=#FF6600]⌘[/color] Worm  [color=#880000]◆[/color] Devourer  [color=#8800CC]▲[/color] AI Overlord", "#808080")
			ui.append_terminal_output("[color=#00FF00]⬛[/color] Base  [color=#FFFFFF]▣[/color] Firewall  [color=#00FF00]⬤[/color] Your Position", "#808080")
			ui.append_terminal_output("Click a node to select. Type 'map travel <ip>' to move there.", "#808080")
		"travel":
			if args.size() < 2:
				ui.append_terminal_output("Usage: map travel <ip>", "#FFFF00")
				return
			var target = args[1]
			_handle_travel([target])
		"zone":
			ui.append_terminal_output("Zone view not yet implemented.", "#FFFF00")
		_:
			ui.append_terminal_output("Usage: map [show|travel|zone] [ip]", "#FFFF00")
...
static func _player_to_dict(player: ZeroDayData.PlayerData) -> Dictionary:
	var d: Dictionary = {}
	if player == null:
		return d
	d["id"] = player.id
