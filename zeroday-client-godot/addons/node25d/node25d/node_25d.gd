...
func _check_view_mode():
    if not Engine.is_editor_hint():
        if Input.is_action_just_pressed(&"view_mode_buttons/45_degree"):
            set_view_mode(0)
        elif Input.is_action_just_pressed(&"view_mode_buttons/isometric"):
            set_view_mode(1)
        elif Input.is_action_just_pressed(&"view_mode_buttons/top_down"):
            set_view_mode(2)
        elif Input.is_action_just_pressed(&"view_mode_buttons/front_side"):
            set_view_mode(3)
        elif Input.is_action_just_pressed(&"view_mode_buttons/oblique_y"):
            set_view_mode(4)
        elif Input.is_action_just_pressed(&"view_mode_buttons/oblique_z"):
            set_view_mode(5)
